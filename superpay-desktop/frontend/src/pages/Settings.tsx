...
export default function Settings() {
  ...
  // Saved custom nodes list (persisted in localStorage)
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      // Always restore the node type if saved, even without a URL
      if (settingsData.monero_node_type) {
        ...
        // Infer type from the URL if type wasn't explicitly saved
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      // Backend wraps in { data: ... }, unwrap it
  ...
  const getNodeStatusIndicator = () => {
    if (!walletStatus?.daemon_connected) {
      return {
        color: 'text-red-400',
    ...
    } else if (walletStatus?.syncing) {
      const blocksRemaining = walletStatus.target_height - walletStatus.height
      return {
        color: 'text-yellow-400',
        bgColor: 'bg-yellow-900/20',
        borderColor: 'border-yellow-700',
        dot: 'bg-yellow-500',
        label: `Syncing (${blocksRemaining.toLocaleString()} blocks remaining)`,
      }
    } else {
      return {
        ...
        dot: 'bg-green-500',
        label: `Connected to ${nodeAddress}`,
      }
    }
  }

  const nodeStatus = getNodeStatusIndicator()
    ...
    <div className="space-y-6 max-w-2xl">
          ...
          {THEMES.map((t) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="absolute left-2 right-2 top-3 h-1.5 rounded" style={{ background: THEME_SWATCH[t.id].bar }} />
                  <span className="absolute left-2 top-6 h-1.5 w-1/2 rounded" style={{ background: THEME_SWATCH[t.id].bar2 }} />
                  <span className="absolute right-2 bottom-2 w-6 h-6 rounded-md" style={{ background: THEME_SWATCH[t.id].chip }} />
                </div>
      ...
      <Card>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              This Computer's Tailscale IP <span className="text-gray-500">(optional)</span>
            </label>
            <input
              type="text"
      ...
      <Card>
        ...
        <div className={`p-4 rounded-lg border mb-6 flex items-center gap-3 ${nodeStatus.bgColor} ${nodeStatus.borderColor}`}>
          <div className={`w-3 h-3 rounded-full ${nodeStatus.dot}`} />
                  ...
                  {nodeType === 'tor' && (
                    <>
                      ...
                      <p className="text-xs text-gray-400 mt-2">
                        Requires Tor running on your Mac: <code className="text-gray-300">brew install tor && tor</code>
                      </p>
                    </>
                  )}
                </div>
