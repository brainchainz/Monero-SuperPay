...
func CreatePairingToken(deps *Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Prefer SERVER_URL (contains the .local hostname like umbrel.local:3033)
		// so POS devices can discover the server on the local network.
		// Fall back to the request Host header if SERVER_URL is a localhost default.
		localBaseURL := deps.Cfg.ServerURL
		if localBaseURL == "" || localBaseURL == "http://localhost:3033" {
			if r.Host != "" {
				scheme := "http"
				if r.TLS != nil {
					scheme = "https"
		...
		if tailscaleIP != "" {
			// Derive the port from SERVER_URL, then request Host, then config
			externalPort := ""
			if u, err := url.Parse(deps.Cfg.ServerURL); err == nil && u.Port() != "" {
				externalPort = u.Port()
			}
			if externalPort == "" && r.Host != "" {
